...
export default function BuildingFile() {
  ...
  async function loadBuilding() {
    ...
    const { data: tenantData } = flatIds.length
      ? await supabase.from('tenants').select('id, full_name, phone, move_in_date, monthly_rent, security_deposit_paid, security_deposit_months, flat_id').eq('status', 'active').in('flat_id', flatIds)
      : { data: [] };
    const tenantByFlat = {};
    (tenantData || []).forEach(t => { tenantByFlat[t.flat_id] = t; });
    const flatsWithTenants = (flatData || []).map(f => ({ ...f, tenants: tenantByFlat[f.id] || null }));
